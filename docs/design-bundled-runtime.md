> ⚠️ **已过时（superseded）**：2026-09-15 的 0.4.2 决定**插件不再内置运行时**。
> 插件包回到约 115KB，首次启动改为从 GitHub Release 下载当前平台的 dsh 运行时包（约 41MB）并原子解包安装，
> 复用既有的热更新链路。本设计里「打进 JAR」「基线解包」「5 平台全内置」等描述仅供历史参考。
> 当前实现见 `DshRuntimeManager`（下载 / 校验 / 解包）与 `.github/workflows/runtime-release.yml`
> （CI 仍产出各平台包 + 可选全平台离线包）。
>
> ---
