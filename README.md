...
- **Send code to Harness**: select code in the editor (or nothing to send the whole file), right-click → `DeepSeek Harness → Send Code to Harness…`. In the dialog you can edit the instruction sent to the agent; on OK the plugin ensures the server is running, submits the code as a prompt to a Harness session, and opens the tool window. The plugin **no longer bundles the dsh runtime** (the plugin archive stays around 167 KB), so on first use it **downloads your platform's runtime package** (≈41 MB, from this repository's GitHub Releases), verifies its sha256, and only then atomically unpacks it locally (into the system temp directory on Windows, `~/.dshstudio/runtime` on macOS/Linux) — **no pre-installed `npx` payload needed**. That usually takes seconds to tens of seconds; a "starting" status in the status bar during that time is normal. You can later check for and download newer dsh versions under `Settings → Tools → DeepSeek Harness` (only your own platform's package, ≈41 MB).
