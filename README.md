...
- **Send code to Harness**: select code in the editor (or nothing to send the whole file), right-click → `DeepSeek Harness → Send Code to Harness…`. In the dialog you can edit the instruction sent to the agent; on OK the plugin ensures the server is running, submits the code as a prompt to a Harness session, and opens the tool window. The plugin **bundles the dsh runtime** (covering Windows, macOS and Linux on both x64 and arm64), so first use only needs to unpack the bundled archive locally (into the system temp directory on Windows, `~/.dshstudio/runtime` on macOS/Linux) — **no network download and no `npx`**. That usually takes seconds to tens of seconds; a "starting" status in the status bar during that time is normal. You can later check for and download newer dsh versions under `Settings → Tools → DeepSeek Harness` (only your own platform's package, ≈41 MB).
