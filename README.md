...
- **启动失败 / 状态红色**：打开 “Server Log” 标签页查看 `dsh web` 输出。常见原因：Node.js 未安装、端口被占用（可改“自动启动端口”）、`npx` 不在 PATH、网络原因导致首次下载 `@deepseek-ai/dsh` 失败。
...
想把插件发布到 JetBrains 插件市场（plugins.jetbrains.com）？完整流程（账号注册、证书生成、签名、上传、审核）见 **[PUBLISHING.md](PUBLISHING.md)**。
