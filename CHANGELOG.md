...
### 新增功能 / New features
- **每个项目一个 dsh 实例 / One dsh instance per project**：把「服务器地址 / 期望端口 / 工作目录 / 鉴权 token」从全局设置下沉到**项目级**（存在项目自己的 workspace 里，不进版本库）。同一个 IDE 里同时打开 A、B 两个项目时，各自跑各自的 dsh 进程、各自监听各自的端口、各自以自己的项目根目录作为 workspace —— 配置互不覆盖，会话互不可见（dsh 的会话按 workspace 隔离）。
- **端口被占用时自动改用空闲端口**：启动前挑一个真正空闲的端口，顺序为「本项目上次分配到的端口 → 设置里的期望端口 → 系统分配的空闲端口」，并把结果写回项目设置，让地址在 IDE 重启后保持稳定。以前两个项目都用默认 3080，第二个要么启动失败，要么误连到第一个项目的服务器上。dsh 的 `webServer.listen` 失败会直接让初始化失败（不会自己换端口），所以这一步必须由插件做。
- **项目关闭自动结束 dsh 进程**：`DshServerManager` 实现 `Disposable`，项目关闭 / IDE 退出时结束本插件为该项目拉起的进程，避免留下孤儿进程继续占端口、继续写共享的 DSH_HOME。新增设置项「项目关闭后保留 dsh 服务器进程」（默认关闭）可按需保留。手动模式连的外部实例不归插件管，不会被误杀。
- 设置页改为**项目级**，并新增「本项目当前地址」实时预览：改地址 / 端口时立刻能看到实际会连的地址，端口被自动换掉时也会说明。
- 启动命令模板里的 `{host}` / `{port}` / `{workdir}` 现在取自**项目级**设置，所以 A、B 两个项目各自的启动命令会带上各自的端口与工作目录。

### 变更 / Changes
- 工作目录默认取**本项目**根目录（`project.basePath`），项目 B 是项目 A 的子目录时各自解析到各自的路径；手动修改只影响当前项目。
- 升级时自动把 0.3.2 及更早存在应用级的服务器配置（地址 / 端口 / 工作目录 / token）迁移到升级后第一个打开的项目，只迁移被用户改过的非默认值。
- 自动模式不再「复用端口上已有的服务」：多项目并行时那多半是另一个项目的 dsh，复用会让两个项目的会话串在一起。需要连已有实例时请在设置里填「服务器地址」（手动模式）。

### 兼容性 / Compatibility
- 其余设置（运行时来源 / 解包位置 / 启动命令模板 / DSH_HOME / 自动启动 / 内嵌浏览器 / 主题 / 背景图）仍是全局的，行为不变。
...
### 修复 / Fixes
- **背景图设置第一次打开就显示 / Background setting now shows on first open**：注入到 dsh 网页「通用设置」面板里的背景图卡片，此前靠「页面上同时出现『外观』和『语言』两行」来判断是否停在通用设置页。这两行由两个独立懒加载的客户端插件提供（dsh-client-ui-theme 提供「外观」、dsh-client-locale 提供「语言」），首次打开设置时它们还没注册，判定因此为假、卡片不注入 —— 切一次语言触发整页重渲染后才出现。现在改为判断设置导航里高亮的是不是「通用设置」（导航由设置外壳自己渲染，不依赖插件加载），并新增 MutationObserver，设置面板一渲染出来就注入卡片，不再等最多 2 秒一次的轮询。
...
### 变更 / Changes
- **运行时改为首次启动下载（插件包回到轻量体积）**：不再把 dsh 运行时打进插件包，首次使用复用热更新链路——从本仓库 GitHub Release 拉取当前平台约 41MB 的包，sha256 校验通过后再原子解包安装并启动。插件包体积、分发与每次小版本发布的下载成本都不受影响；访问不了 GitHub Release 的环境可在 Release 页手动下载「全平台离线包」导入。
- **「运行时来源」去掉「仅内置运行时」选项**：保留「自动（优先已下载运行时，回退系统 dsh）」与「仅系统 dsh（npx）」。

### 新增功能 / New features
- **运行时热更新**：设置页可检查 dsh 新版本，发现后先问再下（约 41MB，只含本机平台），sha256 校验通过后才原子安装，下次启动生效；安装中途断网或被打断不会留下半个版本目录。
- **Node.js 探测与引导**：启动前检测 Node.js，低于依赖包 engines 声明的最低版本时给出提示与安装指引（只提示、不拦截）。

### 说明 / Notes
- 0.4.x 系列为实验性版本，未上架 Marketplace；本次上架版本沿用 0.3.x 序列，编号为 0.3.2。
...
### 说明 / Notes
- 0.3.2 即本次发布版本（2026-09-15）；0.4.x 实验性版本未上架 Marketplace。
