...
intellijPlatform {
    pluginConfiguration {
        ...
        changeNotes = """
            <h3>0.3.3</h3>
            <ul>
              <li><b>每个项目一个 dsh 实例</b>：把「服务器地址 / 端口 / 工作目录 / 鉴权 token」从全局设置下沉到<b>项目级</b>。同一个 IDE 里同时打开 A、B 两个项目时，各自跑各自的 dsh 进程、各自监听各自的端口、各自以自己的项目根目录作为 workspace，配置互不覆盖、会话互不可见。</li>
              <li><b>端口被占用时自动改用空闲端口</b>：以前两个项目都写默认端口 3080，第二个启动会直接失败或误连到第一个项目的服务器上。现在启动前会挑一个真正空闲的端口（优先复用本项目上次分配到的端口，让地址在重启后保持稳定），并明确告诉你「期望端口被占、本项目实际用哪个」。想连已有实例，在设置里填「服务器地址」即可切到手动模式。</li>
              <li><b>项目关闭自动结束 dsh 进程</b>：不再留下孤儿进程继续占端口、继续写共享的 DSH_HOME。手动模式连的外部实例不受影响；确实想让进程在项目关闭后继续跑，可在设置里勾选「项目关闭后保留 dsh 服务器进程」（默认关闭）。</li>
              <li>设置页改为项目级，并新增「本项目当前地址」实时预览，改地址 / 端口时立刻能看到实际会连的地址。</li>
              <li>工作目录默认取<b>本项目</b>根目录：项目 B 是项目 A 的子目录时，各自解析到各自的路径。</li>
              <li>升级时自动把旧版存在全局的应用级服务器配置迁移到升级后第一个打开的项目，不会丢失自定义地址。</li>
            </ul>
            <h3>0.3.2</h3>
            <ul>
              ...
              <li><b>运行时改为首次启动下载（插件包回到轻量体积）</b>：不再把 dsh 运行时打进插件包，首次使用复用热更新链路，从本仓库 GitHub Release 拉取当前平台约 41MB 的包，sha256 校验通过后再原子解包安装并启动；插件包体积、分发与每次小版本发布的下载成本都不受影响。访问不了 GitHub Release 的环境，可在 Release 页手动下载「全平台离线包」导入。</li>
              <li>「运行时来源」去掉了「仅内置运行时」选项，保留「自动（优先已下载运行时，回退系统 dsh）」与「仅系统 dsh（npx）」。</li>
              <li><b>运行时热更新</b>：设置页可检查 dsh 新版本，发现后先问再下（约 41MB，只含本机平台），sha256 校验通过后才原子安装，下次启动生效。</li>
              <li><b>Node.js 探测与引导</b>：启动前检测 Node.js，低于依赖包 engines 声明的最低版本时给出提示与安装指引（只提示、不拦截）。</li>
            </ul>
<h3>0.3.1</h3>
            <ul>
